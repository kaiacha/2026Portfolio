...
export default function VolunteeringWidget() {
  return (
    <div className="glass-effect rounded-[26.46px] p-4 py-3 shadow-xl h-full">
      <div className="flex items-center gap-2 mb-2">
        ...
        />
        <h3 className="text-lg font-semibold text-white/90">Volunteering Experiences</h3>
      </div>
      ...
      <div className="space-y-0 md:space-y-0 mb-0">
        {volunteeringItems.map((item, index) => (
          <div
            key={index}
            className="flex items-center gap-1 rounded-lg hover:bg-white/10 transition-colors cursor-pointer py-0.5 md:py-0.5 md:gap-0"
          >
            <div className="w-7 h-7 md:w-10 md:h-10 rounded-lg flex items-center justify-center text-base md:text-xl flex-shrink-0">
              {item.image}
            </div>
            <div className="flex-1 text-[11px] md:text-xs font-medium text-white/80 leading-snug">
              {item.title}
            </div>
          </div>
        ))}
