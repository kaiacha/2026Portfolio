...
interface RightPanelProps {
  ...
  onRouteSelect: (route: 'fastest' | 'safest') => void
  currentStage: number
  onStageChange: (updater: (prev: number) => number) => void
}

function RightPanel({ selectedRoute, onRouteSelect, currentStage, onStageChange }: RightPanelProps) {
  ...
  const getTimelineStatus = (index: number): 'active' | 'current' | 'inactive' => {
    ...
    <div className={styles.rightPanel}>
      ...
      <div className={styles.routeSelection}>
        ...
        <div className={styles.routesList}>
          <div
            className={selectedRoute === 'fastest' ? styles.routeCardActive : styles.routeCard}
            onClick={() => onRouteSelect('fastest')}
            style={{ cursor: 'pointer' }}
          >
            <div className={styles.routeCardHeader}>
              <div className={styles.routeCardLeft}>
                <div className={selectedRoute === 'fastest' ? styles.routeIconActive : styles.routeIcon}>⚡</div>
                <div className={styles.routeInfo}>
                  <div className={selectedRoute === 'fastest' ? styles.routeNumber : styles.routeNumberInactive}>
                    1. Fastest Route
                  </div>
                  <div className={selectedRoute === 'fastest' ? styles.routeDescription : styles.routeDescriptionInactive}>
                    Speed optimized
                  </div>
                </div>
              </div>
              <div className={selectedRoute === 'fastest' ? styles.routeArrow : styles.routeArrowInactive}>→</div>
            </div>
            <div className={selectedRoute === 'fastest' ? styles.routeCardDivider : styles.routeCardDividerInactive}></div>
            <div className={selectedRoute === 'fastest' ? styles.routeCardDetails : styles.routeCardDetailsInactive}>
              <span>Distance</span>
              <span className={selectedRoute === 'fastest' ? styles.routeCardValue : styles.routeCardValueInactive}>
                6.8 mi • 9m 47s
              </span>
            </div>
          </div>
          <div
            className={selectedRoute === 'safest' ? styles.routeCardActiveSafest : styles.routeCard}
            onClick={() => onRouteSelect('safest')}
            style={{ cursor: 'pointer' }}
          >
            <div className={styles.routeCardHeader}>
              <div className={styles.routeCardLeft}>
                <div className={selectedRoute === 'safest' ? styles.routeIconActiveSafest : styles.routeIcon}>🛡️</div>
                <div className={styles.routeInfo}>
                  <div className={selectedRoute === 'safest' ? styles.routeNumberSafest : styles.routeNumberInactive}>
                    2. Safest Route
                  </div>
                  <div className={selectedRoute === 'safest' ? styles.routeDescriptionSafest : styles.routeDescriptionInactive}>
                    Hazard avoidance
                  </div>
                </div>
              </div>
              <div className={selectedRoute === 'safest' ? styles.routeArrowSafest : styles.routeArrowInactive}>→</div>
            </div>
            <div className={selectedRoute === 'safest' ? styles.routeCardDividerSafest : styles.routeCardDividerInactive}></div>
            <div className={selectedRoute === 'safest' ? styles.routeCardDetailsSafest : styles.routeCardDetailsInactive}>
              <span>Distance</span>
              <span className={selectedRoute === 'safest' ? styles.routeCardValueSafest : styles.routeCardValueInactive}>
                8.1 mi • 12m 18s
              </span>
            </div>
          </div>
            ...
            {timelineStages.map((stage, index) => {
              ...
              return (
                <div key={index} className={styles.timelineItem}>
                  ...
                  >
                    {status === 'active' ? '✓' : status === 'current' ? '●' : index + 1}
                  </div>
                  {!isLast && (
                    <div
                      className={
                  ...
                  <div className={styles.timelineContent}>
                    ...
                    {stage.time && (
                      ...
                      >
                        {status === 'active' ? stage.time : status === 'current' ? stage.time : ''}
                      </div>
                    )}
                  </div>
                </div>
