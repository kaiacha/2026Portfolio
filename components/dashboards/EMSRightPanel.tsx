...
interface EMSRightPanelProps {
  currentStage: number
  onStageChange: (updater: (prev: number) => number) => void
}

function EMSRightPanel({ currentStage, onStageChange }: EMSRightPanelProps) {
  ...
  return (
    <div className={styles.emsRightPanel}>
      ...
      <button className={styles.confirmButton} onClick={handleConfirmReceipt} disabled={currentStage >= 5}>
