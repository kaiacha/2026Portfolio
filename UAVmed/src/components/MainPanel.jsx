...
import mapBgImage from "../images/mapbg.jpg";
import forestDroneViewImage from "../images/forestdroneview.jpg";
import liveCameraFeedVideo from "../images/LiveCameraFeed.mp4";

function MainPanel({ selectedRoute, latitude, longitude }) {
  ...
  const getDronePositionOnRoute = () => {
    const endPoint =
      selectedRoute === "safest" ? safestRouteEnd : fastestRouteEnd;
  ...
  const handleZoomOut = () => {
    setZoomLevel((prev) => Math.max(prev - 0.25, 1)); // Min zoom 1x (full size)
  };

  // Get map image based on selected map type
  const getMapImageStyle = () => {
    const baseStyle = {};

    switch (selectedMapType) {
      case "TERRAIN":
        // Terrain view: more contrast, earth tones
        baseStyle.filter =
          "contrast(1.2) saturate(0.8) hue-rotate(-10deg) brightness(0.9)";
        break;
      case "STREET":
        // Street view: higher brightness, less saturation, more detail
        baseStyle.filter = "contrast(1.1) saturate(0.6) brightness(1.1)";
        break;
      case "SATELLITE":
      default:
        // Satellite view: original image
          ...
          <div className={styles.mapElements}>
            ...
            >
              ...
              <line
                ...
                x2="75"
                y2="20"
                className={`${styles.routeLineFastest} ${
                  selectedRoute === "fastest" ? styles.routeLineActive : ""
                }`}
                stroke="#4caeff"
              ...
              <line
                ...
                x2="60"
                y2="10"
                className={`${styles.routeLineSafest} ${
                  selectedRoute === "safest" ? styles.routeLineActive : ""
                }`}
                stroke="#55ffd4"
            ...
            {/* Route Markers */}
            <div
              className={`${styles.routeMarker1} ${
                selectedRoute === "fastest"
                  ? styles.routeMarkerActive
                  : styles.routeMarkerInactive
              }`}
            >
              1
            </div>
            <div
              className={`${styles.routeMarker2} ${
            ...
            <div className={styles.routesLegend}>
              <div className={styles.routesLabel}>Routes</div>
              <div className={styles.routeItem}>
                <div className={styles.routeLineBlue}></div>
                <span>Fastest - 6.8mi</span>
              </div>
              <div className={styles.routeItem}>
                <div className={styles.routeLineGreen}></div>
                <span>Safest - 8.1mi</span>
              </div>
            </div>
            ...
            {/* Live Camera Feed Overlay - Bottom Right */}
            <div className={styles.cameraOverlayContainer}>
